  ...
  deleteByName(workspaceName).then(function(){
    return create(workspaceName, eventCallback).then(function(client){
      ...
      workspace.activities.list().then(function(response){
        ...
        var offlineActivity = response.activities.find(byName('Offline'));
        ...
        workspace.workflows.create({
          ...
          workflowDeferred.resolve({workflowSid: workflow.sid, activities: {idle: idleActivity.sid, offline: offlineActivity.sid}, workspaceSid: client.workspaceSid});
        }).catch(errorHandler);
        }).catch(errorHandler);
      });
    }).catch(errorHandler);
